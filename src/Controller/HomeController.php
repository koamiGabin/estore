...
class HomeController extends AbstractController
{
    ...
    {
        $articles= $this->getDoctrine()->getRepository(Article::class)->findBy(array(), null, 6, null);
    ...
    public function discussion()
    {
        ...
        * @return HtmlString
        */
        function whatsapp_shop_button(bool $isMobile, string $intlContact, string $url, string $phrase, string $innerText) 
        {
        $text = htmlentities($phrase . ' ' . urlencode($url)) ;
        $href = $isMobile ? "https://wa.me/{$intlContact}?text={$text}":
        "https://web.whatsapp.com/send?phone={$intlContact}&text={$text}";
        // $token = Csrf_Token();
        ...
        return
        preg_match('/(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge
